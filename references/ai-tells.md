...
- **Yoked headline** — a bullet's bold lead-in that pairs a plain action with a rhetorical partner, usually joined by "and" and landing on a flourish with no object of its own. "Shipped product features and argued the debt against them." / "Made the product's blind spots visible and owned." / "Fixed bad data at source instead of downstream." The fact is real; the second clause is where the machine shows. Stop after the fact: "Shipped new features into the product." Mid-paragraph the same shape is ordinary English, so this only applies to headings, bold lead-ins and short standalone lines. (`detect.py` flags this as `yoked-headline`.)
- **Insight frame** — announcing which part of the work was the real one. "The hard part was never the tooling." / "The real work was deciding where AI had no business being." / "The model was the easy half." It reads as a lesson being handed down, and it is the shape a model reaches for when it wants a sentence to sound earned. Say what took the time: "Most of that time went on the engineers rather than the tooling." (`detect.py` flags this as `insight-frame`.)
- **Aphorism** — a line about your own work that reaches for a general law, or an epigram built on a semicolon. "because the constraint is usually somewhere nobody owns" / "The work had moved; nobody had told the system." / ", which is the pattern I took from CrowdStrike". Each is a true observation wearing a proverb, and the fact before it was already doing the work, so delete the clause. An ordinary causal explanation (", which is why staff found it disjointed") is not this tell. (`detect.py` flags this as `aphorism`.)
